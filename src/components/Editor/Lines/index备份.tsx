    ...
    const handleMove = (data: WidgetMoveData) => {
      ...
      info.vertical.forEach(v => {
        ...
        if (nearLine) {
          ...
          // const delta = (data.style.width - movingWidgetSize.width) / 2;
          let result = nearLine.offset;
          if (data.style.rotate) {
            result -= (data.style.width - movingWidgetSize.width) / 2;
          }
          if (v.type === 'vm') {
            result= nearLine.offset - movingWidgetSize.width / 2;
            if (data.style.rotate) {
              result -= (data.style.width - movingWidgetSize.width) / 2;
            }
          } else if (v.type === 'vr') {
            result= nearLine.offset - movingWidgetSize.width;
            if (data.style.rotate) {
              result -= (data.style.width - movingWidgetSize.width) / 2;
            }
          }
      ...
      info.horizontal.forEach(h => {
        ...
        if (nearLine) {
          ...
          let result = nearLine.offset;
          if (data.style.rotate) {
            // result= nearLine.offset + cos(data.style.rotate) * data.style.height;
            result += (movingWidgetSize.height - data.style.height) / 2;
          }
          // console.log('def', nearLine.offset, result);
          if (h.type === 'hm') {
            result= nearLine.offset - movingWidgetSize.height / 2;
            if (data.style.rotate) {
              result += (movingWidgetSize.height - data.style.height) / 2;
            }
          } else if (h.type === 'hb') {
            result= nearLine.offset - movingWidgetSize.height;
            if (data.style.rotate) {
              result += (movingWidgetSize.height - data.style.height) / 2;
              // result += movingWidgetSize.width;
            }
            // console.log('hbhb', nearLine.offset, movingWidgetSize.height, result);
          }
